...
describe CurrencyCloud do
  ...
  describe ".session" do
    ...
    it "returns a session object" do
      CurrencyCloud.environment = :test
      CurrencyCloud.login_id = 'test@example.com'
      CurrencyCloud.api_key = 'e3b0d6895f91f46d9eaf5c95aa0f64dca9007b7ab0778721b6cdc0a8bc7c563b'
    ...
    it "raises an error if the environment is not set" do
      ...
      expect { CurrencyCloud.session }
       .to raise_error(CurrencyCloud::AuthenticationError, "'' is not a valid environment, must be one of: production, demonstration, uat, test")
    end
    
    it "raises an error if the environment is invalid" do
      CurrencyCloud.environment = :invalid
      expect { CurrencyCloud.session }
       .to raise_error(CurrencyCloud::AuthenticationError, "'invalid' is not a valid environment, must be one of: production, demonstration, uat, test")
    end
    
    it "raises an error if the login_id is not set" do
      CurrencyCloud.environment = :test
      expect { CurrencyCloud.session }
       .to raise_error(CurrencyCloud::AuthenticationError, "login_id must be set using CurrencyCloud.login_id=")
    end
    ...
    it "raises an error if the api_key is not set" do
      ...
      CurrencyCloud.login_id = 'test@example.com'
      expect { CurrencyCloud.session }
       .to raise_error(CurrencyCloud::AuthenticationError, "api_key must be set using CurrencyCloud.api_key=")
    end
